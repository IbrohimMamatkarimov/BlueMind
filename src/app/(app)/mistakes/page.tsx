...
function MistakeItem({ item, checked, starting, onToggle, onPractice, onJournalSaved }: {
  ...
  const hasJournal = Boolean(item.journal?.reason || item.journal?.warning);
  const changed = reason.trim() !== (item.journal?.reason ?? "") || warning.trim() !== (item.journal?.warning ?? "");
  ...
  async function saveJournal() {
    ...
    try {
      const response = await fetch("/api/mistakes", {
        ...
        body: JSON.stringify({ section: item.section, questionId: item.questionId, reason, warning }),
      });
    ...
    <div className="flex flex-col gap-4 sm:flex-row">
      <div className="flex min-w-0 flex-1 gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-brand-slate mt-2">Missed {item.mistakeCount} time{item.mistakeCount === 1 ? "" : "s"} · {item.attemptCount} total attempt{item.attemptCount === 1 ? "" : "s"} · Last tried {new Date(item.lastAttemptAt).toLocaleDateString()}</p>
        </div>
      </div>
